    ...
    *,
    ...
    if selected_tab == "⚡ Urgent":
        ...
        for oid in order_ids:
            ctx = _load_order_context(int(venue_id), int(oid))
            when = getattr(ctx.order, "created_at", None)
            when_s = when.strftime("%Y-%m-%d %H:%M") if isinstance(when, datetime) else ""
            title = f"#{oid}" + (f" · {when_s}" if when_s else "") + f" · {len(grouped[oid])} requests"

            expanded = (deep_order_id is not None and int(deep_order_id) == int(oid))
            with st.expander(title, expanded=expanded):
                set_query_params(page="tracking", order_id=str(int(oid)))
                _render_urgent_requests_for_order(ctx, grouped[oid], key_prefix=f"urg_{int(oid)}")
        return
