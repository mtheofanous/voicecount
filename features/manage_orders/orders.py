...
from domain.models import Order, OrderLine, Product, Provider, ProviderReceipt, ProviderDiscountRule, ProviderSendStatus, OrderPresence
...
def _render_send_section(
    *,
    ...
    with st.expander("📤 Generar envíos", expanded=True):
        ...
        lang = (_safe_str(getattr(v, "email_lang", None)) or "es").strip().lower()
...
#     from domain.models import ProviderLineFollowUp
...
#     # -------------------------------------------------------------------------
#     # Load followups once (single query)
#     # -------------------------------------------------------------------------
#     with get_session() as s:
#         fus = list(
#             s.exec(
...
#         )
#     fu_by_line_id = {
#         int(getattr(x, "order_line_id")): x
#         for x in fus
#         if getattr(x, "order_line_id", None) is not None
#     }

#     STATUS_OPTS = {"ok", "partial", "missing"}

#     def _status_clean(x) -> str:
#         v = (str(x) if x is not None else "ok").strip().lower()
#         return v if v in STATUS_OPTS else "ok"
...
#     def _line_meta(line_id: int) -> dict:
#         ln = next((x for x in lines if int(getattr(x, "id", 0) or 0) == int(line_id)), None)
#         pid = int(getattr(ln, "product_id", 0) or 0) if ln else 0
#         p = products_by_id.get(pid) if pid else None

#         name = (
#             _safe_str(getattr(p, "name", None))
#             or _safe_str(getattr(ln, "matched_name", None) or getattr(ln, "spoken_name", None))
#             or "Producto"
#         )
#         desc = _safe_str(getattr(p, "description", None))
#         unit = _safe_str(getattr(p, "unit", None)) or _safe_str(getattr(ln, "unit", None)) or "unidad"
#         qty = _safe_float(getattr(ln, "quantity", 0.0), 0.0)
#         return {"name": name, "desc": desc, "unit": unit, "qty": qty}

#     def _upsert_venue_followups(rows: List[dict]) -> None:
#         now = _now()
#         with get_session() as s:
#             for r in rows:
#                 fu = s.get(ProviderLineFollowUp, int(r["followup_id"]))
#                 if not fu:
#                     continue
#                 fu.venue_qty = _safe_float(r.get("venue_qty"), 0.0)
#                 fu.venue_comment = (_safe_str(r.get("venue_comment")) or None)
#                 fu.updated_at = now
#                 s.add(fu)
#             s.commit()
...
#         rr = receipts.get(_norm_provider(prov))
#         if rr and bool(getattr(rr, "received", False)):
#             received_count += 1
...
#     # -------------------------------------------------------------------------
#     # Provider cards (minimal + friendly)
#     # -------------------------------------------------------------------------
#     for prov in providers:
#         prov_key = _norm_provider(prov)
#         rr = receipts.get(prov_key)
#         is_received = bool(rr and getattr(rr, "received", False))
#         badge = "✅ Recibido" if is_received else "⏳ Pendiente"

#         # Build a minimal table for this provider
#         provider_line_ids = [
#             int(x.get("line_id"))
#             for x in grouped.get(prov, [])
#             if int(x.get("line_id") or 0) > 0
#         ]

#         rows = []
#         declared_issues = 0

#         for lid in provider_line_ids:
#             meta = _line_meta(lid)
#             fu = fu_by_line_id.get(lid)

#             qty_ordered = _safe_float(getattr(fu, "qty_ordered", None), meta["qty"])
#             supplier_status = _status_clean(getattr(fu, "supplier_status", "ok") if fu else "ok")
#             supplier_qty = _safe_float(getattr(fu, "supplier_qty", None), qty_ordered) if fu else qty_ordered
#             supplier_reason = _safe_str(getattr(fu, "supplier_reason", "")) if fu else ""
#             supplier_comment = _safe_str(getattr(fu, "supplier_comment", "")) if fu else ""

#             aviso = ""
#             if supplier_status != "ok":
#                 declared_issues += 1
#                 # compact, one-line, easy to scan
#                 reason_part = f" · {supplier_reason}" if supplier_reason else ""
#                 aviso = f"{supplier_status.upper()} · {supplier_qty}/{qty_ordered}{reason_part}"

#             rows.append(
#                 {
#                     "followup_id": int(getattr(fu, "id")) if fu and getattr(fu, "id", None) is not None else 0,
#                     "Producto": meta["name"],
#                     "Unidad": meta["unit"],
#                     "Pedido": float(qty_ordered),
#                     "Aviso proveedor": aviso,  # empty when OK
#                     "Recibido": float(_safe_float(getattr(fu, "venue_qty", None), 0.0) if fu else 0.0),
#                     "Comentario local": _safe_str(getattr(fu, "venue_comment", "")) if fu else "",
#                     # keep hidden by default; shown only with toggle
#                     "Descripción": meta["desc"],
#                     "Comentario proveedor": supplier_comment,
#                 }
#             )

#         df = pd.DataFrame(rows)

#         issues_badge = f"⚠️ {declared_issues} aviso(s)" if declared_issues else "🟢 Sin avisos"
#         st.markdown(
#             f"""
#             <div class='voi-card'>
#               <div class='voi-row' style='justify-content:space-between;'>
#                 <div>
#                   <div style='font-weight:800;font-size:1.05rem'>{prov_key}</div>
#                   <div class='voi-muted'>{len(rows)} líneas · {issues_badge}</div>
#                 </div>
#                 <div class='voi-chip'>{badge}</div>
#               </div>
#             </div>
#             """,
#             unsafe_allow_html=True,
...
#             # Minimal toggles (UX)
#             t1, t2, t3 = st.columns([1, 1, 2], vertical_alignment="center")
#             with t1:
#                 show_desc = st.toggle("Mostrar descripción", value=False, key=f"show_desc_{int(order.id)}_{prov_key}")
#             with t2:
#                 show_supplier_comments = st.toggle("Mostrar comentarios proveedor", value=False, key=f"show_supc_{int(order.id)}_{prov_key}")
#             with t3:
#                 st.caption("Tip: si no hay avisos, puedes centrarte solo en “Recibido” + comentario.")

#             # Receipt note
#             note_key = f"recv_note_{int(order.id)}_{prov_key}"
#             note_val = _safe_str(getattr(rr, "note", "")) if rr else ""
#             note = st.text_area("Nota (opcional)", value=note_val, key=note_key, height=70)

#             if df.empty:
#                 st.info("Sin líneas.")
#                 continue

#             # Decide visible columns (minimal default)
#             visible_cols = ["Producto", "Unidad", "Pedido", "Aviso proveedor", "Recibido", "Comentario local"]
#             if show_desc:
#                 visible_cols.insert(1, "Descripción")
#             if show_supplier_comments:
#                 visible_cols.append("Comentario proveedor")

#             df_view = df[["followup_id"] + visible_cols].copy()

#             edited = st.data_editor(
#                 df_view,
#                 hide_index=True,
#                 width="stretch",
#                 disabled=[c for c in df_view.columns if c not in {"Recibido", "Comentario local"}],
#                 column_config={
...
#                     "Aviso proveedor": st.column_config.TextColumn("Aviso proveedor", width="large"),
...
#                 },
#                 key=f"recv_editor_{int(order.id)}_{prov_key}",
#             )

#             # Actions (clean bar)
#             # Actions (clean bar)
#             a1, a2, a3, a4 = st.columns([1.2, 1.2, 1.6, 1.6], vertical_alignment="center")

#             with a1:
#                 if st.button("💾 Guardar", type="primary", width="stretch", key=f"save_recv_{int(order.id)}_{prov_key}"):
#                     payload = []
#                     for _, r in edited.iterrows():
#                         fid = int(r.get("followup_id") or 0)
#                         if fid <= 0:
#                             continue
#                         payload.append(
#                             {
#                                 "followup_id": fid,
#                                 "venue_qty": _safe_float(r.get("Recibido"), 0.0),
#                                 "venue_comment": _safe_str(r.get("Comentario local")),
#                             }
#                         )
#                     _upsert_venue_followups(payload)
#                     _bump_refresh(venue_id)
#                     st.success("Guardado ✓")
#                     st.rerun()

#             with a2:
#                 if st.button("✅ Marcar recibido", width="stretch", key=f"recv_yes_{int(order.id)}_{prov_key}"):
...
#             with a3:
#                 if st.button("↩️ Marcar pendiente", width="stretch", key=f"recv_no_{int(order.id)}_{prov_key}"):
#                     _upsert_provider_receipt(
#                         venue_id=venue_id,
#                         order_id=int(order.id),
#                         provider_name=prov_key,
...
#             with a4:
#                 venue_url = build_seguimiento_url(
#                     order_id=int(order.id),
#                     provider_name=norm_provider(prov_key),
#                     role=ROLE_VENUE,
...
def _render_receive_section(
    *,
    ...
    import pandas as pd
    from sqlmodel import select
    from domain.models import ProviderLineFollowUp, SeguimientoTicket

    # ✅ Try to import ticket table (degrade gracefully if not present)
    try:
        from features.seguimiento.seguimiento import SeguimientoTicket  # type: ignore
    except Exception:
        SeguimientoTicket = None  # type: ignore
    ...
    # -------------------------------------------------------------------------
    # Modern micro-CSS (keeps your existing card style but improves density)
    # -------------------------------------------------------------------------
    st.markdown(
        """
        <style>
        .rx-card{border:1px solid rgba(49,51,63,.12); border-radius:18px; padding:14px 14px; margin:10px 0; background:rgba(255,255,255,.03);}
        .rx-head{display:flex; align-items:center; justify-content:space-between; gap:12px;}
        .rx-title{font-weight:800; font-size:1.05rem;}
        .rx-sub{opacity:.7; font-size:.9rem; margin-top:2px;}
        .rx-badges{display:flex; gap:8px; align-items:center; flex-wrap:wrap;}
        .rx-badge{display:inline-block; padding:4px 10px; border-radius:999px; border:1px solid rgba(49,51,63,.18); font-size:.85rem; opacity:.95;}
        .rx-badge-ok{background:rgba(46,204,113,.12); border-color:rgba(46,204,113,.35);}
        .rx-badge-warn{background:rgba(241,196,15,.14); border-color:rgba(241,196,15,.35);}
        .rx-badge-open{background:rgba(231,76,60,.12); border-color:rgba(231,76,60,.35);}
        .rx-badge-rec{background:rgba(33,150,243,.10); border-color:rgba(33,150,243,.35);}
        .rx-mini{margin-top:10px; padding:10px 10px; border-radius:12px; border:1px dashed rgba(49,51,63,.18); opacity:.95;}
        .rx-mini li{margin:2px 0;}
        </style>
        """,
        unsafe_allow_html=True,
    )
    ...
    # -------------------------------------------------------------------------
    # Tickets (one query) -> group per provider
    # -------------------------------------------------------------------------
    tickets_by_provider: dict[str, list] = {}
    open_count_by_provider: dict[str, int] = {}

    if SeguimientoTicket is not None:
        with get_session() as s:
            tks = list(
                s.exec(
        ...
        for t in tks:
            prov = _norm_provider(getattr(t, "provider_name", ""))
            tickets_by_provider.setdefault(prov, []).append(t)

        for prov, tlist in tickets_by_provider.items():
            open_count_by_provider[prov] = sum(1 for t in tlist if (getattr(t, "state", "open") or "open") == "open")

    # -------------------------------------------------------------------------
    # Global overview (progress + KPIs)
    # -------------------------------------------------------------------------
    providers = list(grouped.keys())

    received_count = 0
    open_tickets_total = 0
    supplier_warn_total = 0
    ...
    for prov in providers:
        ...
        open_tickets_total += int(open_count_by_provider.get(prov_key, 0) or 0)

        # supplier warnings counted from followups (fast scan)
        provider_line_ids = [
            int(x.get("line_id"))
            for x in grouped.get(prov, [])
            if int(x.get("line_id") or 0) > 0
        ...
        for lid in provider_line_ids:
            fu = fu_by_line_id.get(lid)
            stt = _status_clean(getattr(fu, "supplier_status", "ok") if fu else "ok")
            if stt != "ok":
                supplier_warn_total += 1

    st.progress(received_count / max(1, len(providers)))
    k1, k2, k3 = st.columns([1.2, 1.2, 1.6], vertical_alignment="center")
    with k1:
        st.metric("Proveedores recibidos", f"{received_count}/{len(providers)}")
    with k2:
        st.metric("Avisos proveedor (líneas)", supplier_warn_total)
    with k3:
        st.metric("Tickets abiertos", open_tickets_total)

    # Global filter controls
    f1, f2, f3 = st.columns([1.4, 1.4, 2.2], vertical_alignment="center")
    with f1:
        only_with_issues = st.toggle("Solo con incidencias", value=False, key=f"rx_only_issues_{int(order.id)}")
    with f2:
        only_pending = st.toggle("Solo pendientes", value=False, key=f"rx_only_pending_{int(order.id)}")
    with f3:
        st.caption("Tip: “Solo con incidencias” es el modo rápido para operar cada día.")
    ...
    for prov in providers:
        ...
        rr = receipts.get(prov_key)
        is_received = bool(rr and getattr(rr, "received", False))
        badge_rec = "✅ Recibido" if is_received else "⏳ Pendiente"

        # Build provider line rows (minimal)
        provider_line_ids = [
            int(x.get("line_id"))
            for x in grouped.get(prov, [])
            if int(x.get("line_id") or 0) > 0
        ...
        # Count supplier warnings + compute quick mismatch count
        declared_issues = 0
        mismatches = 0
        ...
        for lid in provider_line_ids:
            ...
            venue_qty = _safe_float(getattr(fu, "venue_qty", None), 0.0) if fu else 0.0

            expected = supplier_qty if supplier_qty is not None else qty_ordered

            if supplier_status != "ok":
                declared_issues += 1
            # mismatch if venue has entered and differs from expected (or supplier warned)
            if (venue_qty > 0 and abs(venue_qty - expected) > 1e-9) or (supplier_status != "ok"):
                mismatches += 1

            aviso = ""
            if supplier_status != "ok":
                reason = _safe_str(getattr(fu, "supplier_reason", "")) if fu else ""
                reason_part = f" · {reason}" if reason else ""
                aviso = f"{supplier_status.upper()} · {supplier_qty:g}/{qty_ordered:g}{reason_part}"
            ...
            rows.append(
                {
                    ...
                    "Unidad": meta["unit"],
                    "Pedido": float(qty_ordered),
                    "Aviso proveedor": aviso,
                    "Recibido": float(venue_qty),
                    "Comentario local": _safe_str(getattr(fu, "venue_comment", "")) if fu else "",
                    "Descripción": meta["desc"],
                    "Comentario proveedor": _safe_str(getattr(fu, "supplier_comment", "")) if fu else "",
                }
            )

        df = pd.DataFrame(rows)

        open_tickets = int(open_count_by_provider.get(prov_key, 0) or 0)
        has_any_issue = (declared_issues > 0) or (open_tickets > 0) or (mismatches > 0)

        if only_with_issues and not has_any_issue:
            continue
        if only_pending and is_received:
            continue

        # Build nice badges
        badges = []
        if open_tickets > 0:
            badges.append(f"<span class='rx-badge rx-badge-open'>🎫 {open_tickets} ticket(s)</span>")
        if declared_issues > 0:
            badges.append(f"<span class='rx-badge rx-badge-warn'>⚠️ {declared_issues} aviso(s)</span>")
        if declared_issues == 0 and open_tickets == 0:
            badges.append("<span class='rx-badge rx-badge-ok'>🟢 Sin incidencias</span>")

        badges.append(
            f"<span class='rx-badge rx-badge-rec'>{badge_rec}</span>"
        )

        # Seguimiento link (venue view) to fix tickets
        venue_url = build_seguimiento_url(
            order_id=int(order.id),
            provider_name=norm_provider(prov_key),
            role=ROLE_VENUE,
        ...
        st.markdown(
            ...
            <div class='rx-card'>
              <div class='rx-head'>
                <div>
                  <div class='rx-title'>{prov_key}</div>
                  <div class='rx-sub'>{len(rows)} líneas · {mismatches} con posible diferencia</div>
                </div>
                <div class='rx-badges'>
                  {"".join(badges)}
                </div>
        ...
        # Modern action bar
        a1, a2, a3 = st.columns([1.6, 1.2, 1.2], vertical_alignment="center")
        with a1:
            st.link_button(
                "🛠️ Revisar / resolver incidencias",
                venue_url,
                use_container_width=True,
                type="primary" if has_any_issue else "secondary",
            )
        with a2:
            if st.button(
                "✅ Marcar recibido",
                use_container_width=True,
                key=f"rx_mark_received_{int(order.id)}_{prov_key}",
            ):
                note_key = f"rx_note_{int(order.id)}_{prov_key}"
                note_val = _safe_str(st.session_state.get(note_key, ""))
                _upsert_provider_receipt(
                    venue_id=venue_id,
                    order_id=int(order.id),
                    provider_name=prov_key,
        ...
        with a3:
            if st.button(
                "↩️ Marcar pendiente",
                use_container_width=True,
                key=f"rx_mark_pending_{int(order.id)}_{prov_key}",
            ):
                note_key = f"rx_note_{int(order.id)}_{prov_key}"
                note_val = _safe_str(st.session_state.get(note_key, ""))
                _upsert_provider_receipt(
        ...
        # Ticket mini-preview (top 3 open tickets) without leaving the Orders screen
        if SeguimientoTicket is not None and open_tickets > 0:
            tlist = tickets_by_provider.get(prov_key, [])
            open_list = [t for t in tlist if (getattr(t, "state", "open") or "open") == "open"]
            open_list = sorted(open_list, key=lambda t: getattr(t, "updated_at", datetime.utcnow()), reverse=True)
            preview = open_list[:3]

            items = []
            for t in preview:
                pname = _safe_str(getattr(t, "product_name", "")) or "Producto"
                kind = _safe_str(getattr(t, "kind", "")) or "ticket"
                note = _safe_str(getattr(t, "note", ""))

                # short, scannable line
                short = note[:90] + ("…" if len(note) > 90 else "")
                items.append(f"<li><b>{pname}</b> · <code>{kind}</code> · {short}</li>")

            st.markdown(
                "<div class='rx-mini'><b>🎫 Incidencias abiertas (preview)</b><ul>"
                + "".join(items)
                + "</ul></div>",
                unsafe_allow_html=True,
            )

        # Expandable: reception editor (only when needed)
        # Default: keep it closed if no issues AND already received
        expand_default = (not is_received) or has_any_issue
        with st.expander("📋 Abrir recepción (detalle)", expanded=expand_default):
            t1, t2, t3 = st.columns([1, 1, 2], vertical_alignment="center")
            with t1:
                show_desc = st.toggle(
                    "Mostrar descripción",
                    value=False,
                    key=f"show_desc_{int(order.id)}_{prov_key}",
                )
            with t2:
                show_supplier_comments = st.toggle(
                    "Mostrar comentarios proveedor",
                    value=False,
                    key=f"show_supc_{int(order.id)}_{prov_key}",
                )
            with t3:
                st.caption("Modo rápido: edita solo “Recibido” y guarda. Usa Seguimiento para tickets/factura.")

            # Note (stored on receipt row)
            note_key = f"rx_note_{int(order.id)}_{prov_key}"
            st.session_state.setdefault(note_key, _safe_str(getattr(rr, "note", "")) if rr else "")
            st.text_area("Nota (opcional)", key=note_key, height=70)
            ...
            # ✅ Modern: only allow editing received/comment
            edited = st.data_editor(
                df_view,
                hide_index=True,
                width="stretch",
            ...
            with b1:
                if st.button(
                    ...
                    type="primary",
                    use_container_width=True,
                    key=f"save_recv_{int(order.id)}_{prov_key}",
                ):
                    payload = []
                    for _, r in edited.iterrows():
                        fid = int(r.get("followup_id") or 0)
                    ...
                    st.rerun()
            with b2:
                st.caption("Para incidencias + factura: usa el botón “Revisar / resolver incidencias” (Seguimiento).")
