        ...
        for lid, payload in per_line.items():
            row = s.exec(
                select(ProviderLineFollowUp).where(
                    ...
                    ProviderLineFollowUp.order_line_id == int(lid),
                )
            ).first()
            if not row:
            ...
            # store invoice-side info too (so ticket creation can be re-checked later)
            row.invoice_listed = bool(payload.get('listed', True))
            row.invoice_qty = float(payload.get('qty_invoiced') or 0.0)
            row.updated_at = _now()
            row.updated_by = 'venue'
            s.add(row)
        s.commit()
