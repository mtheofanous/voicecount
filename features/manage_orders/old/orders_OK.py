...
def orders_tab(venue_id: int, venue_role: Optional[str]) -> None:
    ...
    with top1:
        status_filter = st.radio(
            ...
            horizontal=True,
            label_visibility="collapsed",
            format_func=lambda x: {"draft": "Borradores", "ready_to_send": "Listo", "pending_receive": "Pendiente", "final": "Historial"}.get(x, x),
    ...
    if status == "draft":
        ...
        _render_pending_section(venue_id=venue_id, order=order, products=products, lines=lines)
    else:
