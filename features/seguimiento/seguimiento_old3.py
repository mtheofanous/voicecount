...
def save_supplier_resolutions(
    ...
    with get_session() as s:
        ...
        for tid, payload in (per_ticket or {}).items():
            ...
            res = (payload or {}).get("resolution") or "no_action"
            ...
            # State handling
            if res in {"credit_note", "supplementary_delivery", "no_action"}:
...
def _render_supplier_resolution(ctx: Dict[str, Any]) -> None:
    ...
    # 1) Per-item resolution selection
    for t in open_t:
        ...
        with st.container(border=True):
            ...
            res = st.selectbox(
                ...
                index=["credit_note", "supplementary_delivery", "no_action"].index(st.session_state[key]),
                key=key,
                format_func=lambda x: {
                    "credit_note": "📝 Credit note",
                    "supplementary_delivery": "🚚 Re-delivery",
                    "no_action": "⛔ No action (we cannot do it)",
                }[x],
            )
