...
# =============================================================================
# Follow-ups (per-line) + receipt header
# =============================================================================

def _upsert_followups_supplier(
    *,
    venue_id: int,
    order_id: int,
    provider_name: str,
    ...
    """Upsert ProviderLineFollowUp rows from supplier-side inputs."""
    provider_name = _s(provider_name)
    touched = 0
    with get_session() as s:
        for lid, payload in (lines_payload or {}).items():
            lid_i = int(lid)
            row = s.exec(
                select(ProviderLineFollowUp).where(
                    ProviderLineFollowUp.venue_id == venue_id,
            ...
            row.qty_ordered = float(payload.get("qty_ordered") or 0.0)
            row.supplier_status = _s(payload.get("supplier_status") or "") or "ok"
            row.supplier_qty = float(payload.get("supplier_qty") or 0.0)
...
def _upsert_followups_venue(
    *,
    ...
    """Upsert venue-side received qty into ProviderLineFollowUp."""
    provider_name = _s(provider_name)
    touched = 0
    with get_session() as s:
        for lid, payload in (per_line or {}).items():
            ...
            # Keep qty_ordered if provided
            if payload.get("qty_ordered") is not None:
                row.qty_ordered = float(payload.get("qty_ordered") or 0.0)

            row.venue_qty = float(payload.get("qty_received") or 0.0)
            row.venue_comment = _s(comment) or None
            row.updated_at = _now()
            row.updated_by = _s(actor) or None
            s.add(row)
            touched += 1
...
def _upsert_receipt_header(
    *,
    ...
    provider_name: str,
    actor: str,
    supplier_declaration: Optional[str] = None,   # "full"|"partial"|"none"
    supplier_comment: Optional[str] = None,
    invoice_number: Optional[str] = None,
    received: Optional[bool] = None,
    note: Optional[str] = None,
) -> None:
    """Upsert ProviderReceipt header so the pending panel can show invoice + received status."""
    provider_name = _s(provider_name)
    with get_session() as s:
        row = s.exec(
            select(ProviderReceipt).where(
                ProviderReceipt.venue_id == venue_id,
        ...
        if supplier_declaration:
            row.supplier_declaration = supplier_declaration
            row.supplier_declared_at = _now()
            row.supplier_declared_by = _s(actor) or None
            row.supplier_declared_comment = _s(supplier_comment) or None

        if invoice_number is not None and _s(invoice_number):
            row.invoice_number = _s(invoice_number)
            row.invoice_number_set_at = _now()
            row.invoice_number_set_by = _s(actor) or None

        if received is not None:
            row.received = bool(received)
            row.received_at = _now() if received else None
            row.received_by = _s(actor) or None

        if note is not None and _s(note):
            row.note = _s(note)
...
def _header(*, title: str, wf: OrderWorkflow, order: Order, provider_name: str, venue_name: str = '', invoice_number: str = '') -> None:
    st.title(title)
    vn = (venue_name or os.getenv('VOICECOUNT_VENUE_NAME') or '').strip()
    if not vn:
        vn = f"Venue {int(order.venue_id)}"
    inv = (invoice_number or '').strip()
    cap = f"**{vn}** · Pedido #{int(order.id)} · Proveedor: **{provider_name}**"
    if inv:
        cap += f" · Factura: **{inv}**"
    st.caption(cap)
...
def _load_receipt_header(*, venue_id: int, order_id: int, provider_name: str) -> Optional[ProviderReceipt]:
    with get_session() as s:
        return s.exec(
            select(ProviderReceipt).where(
                ProviderReceipt.venue_id == int(venue_id),
                ProviderReceipt.order_id == int(order_id),
                ProviderReceipt.provider_name == norm_provider(provider_name),
            )
        ).first()
# =============================================================================
# UI: Supplier
...
def _supplier_ui(*, order: Order, provider_name: str, wf: OrderWorkflow, lines: list[OrderLine], prod_map: dict[int, Product]) -> None:
    receipt = _load_receipt_header(venue_id=int(order.venue_id), order_id=int(order.id), provider_name=provider_name)
    inv_no = _s(getattr(receipt, 'invoice_number', '')) if receipt else ''
    venue_q = st.query_params.get('venue') or st.query_params.get('venue_name') or ''
    _header(title="Confirmación del proveedor", wf=wf, order=order, provider_name=provider_name, venue_name=_s(venue_q), invoice_number=inv_no)
    ...
    if wf.state == "ORDER_SENT":
        ...
        if st.button("Guardar", type="primary", use_container_width=True, key=f"{base_key}_save"):
            # 1) Persist per-line declaration so the Orders → Pendiente panel can show it.
            _upsert_followups_supplier(
            ...
            # 2) Persist a lightweight header (supplier_declaration) for convenience.
            decl = "partial"
            if to_state == "SUPPLIER_CONFIRMED_FULL":
                decl = "full"
            elif to_state == "SUPPLIER_CONFIRMED_NONE":
                decl = "none"
            _upsert_receipt_header(
                ...
                provider_name=provider_name,
                actor="supplier",
                supplier_declaration=decl,
                supplier_comment=_s(comment) or None,
            )

            # lightweight audit note (no prices)
...
def _venue_ui(
    *,
    ...
    inv_no = _s(getattr(receipt, 'invoice_number', '')) if receipt else ''
    venue_q = st.query_params.get('venue') or st.query_params.get('venue_name') or ''
    _header(title="Seguimiento (local)", wf=wf, order=order, provider_name=provider_name, venue_name=_s(venue_q), invoice_number=inv_no)
    ...
    if wf.state in {"SUPPLIER_CONFIRMED_FULL", "SUPPLIER_CONFIRMED_PARTIAL", "SUPPLIER_CONFIRMED_NONE"}:
        ...
        if st.button("Marcar como RECIBIDO", type="primary", use_container_width=True):
            # Header: mark as physically received (invoice matching comes next).
            _upsert_receipt_header(
                venue_id=int(order.venue_id),
                order_id=int(order.id),
                provider_name=provider_name,
        ...
        invoice_number = st.text_input("Nº de factura (opcional)", placeholder="Ej.: F-2026-00123")
        ...
        with col_a:
            if st.button("✅ Todo coincide → Cerrar", type="primary", use_container_width=True, key="close_match"):
                if any_not_listed or any_discrepancy:
                    ...
                    # Persist invoice header + received quantities (so Orders → Pendiente reflects reality).
                    _upsert_receipt_header(
                        venue_id=int(order.venue_id),
                        order_id=int(order.id),
                        provider_name=provider_name,
        ...
        with col_b:
            if st.button("⚠️ Hay problema → Abrir incidencia", type="secondary", use_container_width=True, key="open_issue"):
                # Always persist what the venue saw (invoice number + received qty), even if it's a problem.
                _upsert_receipt_header(
                    venue_id=int(order.venue_id),
                    order_id=int(order.id),
                    provider_name=provider_name,
                ...
                affected = {lid: v for lid, v in per_line.items() if v.get("listed") and float(v.get("qty_invoiced") or 0.0) > float(v.get("qty_received") or 0.0) + 1e-9}
