...
from features.manage_orders.orders import ensure_orders_migrations, orders_tab_modern
...
def ensure_sqlite_columns() -> None:
    """Tiny SQLite migrations so older DBs don't crash when models change.
    Avoid SQLModel.metadata.create_all(engine) because it can recreate global indexes and crash in SQLite.
    """

    with engine.begin() as conn:
        # ✅ Create only the tables we actually rely on (no global metadata)
        # If tables already exist, checkfirst=True makes this safe.
        try:
            Product.__table__.create(conn, checkfirst=True)
        except Exception:
            pass
        try:
            Order.__table__.create(conn, checkfirst=True)
        except Exception:
            pass
        try:
            OrderLine.__table__.create(conn, checkfirst=True)
        except Exception:
            pass
...
def new_order_tab(venue_id: int, role: str | None = None):
    ...
    # Chat-like state (WhatsApp-ish)
    if "order_chat" not in st.session_state:
        st.session_state["order_chat"] = []  # list[dict]: {"role":"user"|"asr", "text": str}
    ...
    # ---------------------------
    # Modern WhatsApp-ish CSS
    # ---------------------------
    st.markdown(
        """
        <style>
          .block-container { padding-top: 1.1rem; padding-bottom: 6rem; max-width: 980px; }
    ...
    # Helper: append message to chat + keep transcript_area as parser source
    def _append_message(role_: str, text_: str):
        text_ = (text_ or "").strip()
        if not text_:
            return
        st.session_state["order_chat"].append({"role": role_, "text": text_})
    ...
    with hL:
        ...
        st.caption("Escribe como en WhatsApp o dicta con el micro. Se acumula todo para parsear.")
    with hR:
        clear_clicked = st.button("🧹 Limpiar", key="btn_clear_all_chat")
        if clear_clicked:
            _clear_all(used_component_recorder=True)
    ...
    if not st.session_state["order_chat"]:
        st.info("Empieza escribiendo abajo o graba un audio 👇")

    for msg in st.session_state["order_chat"]:
    ...
    # =========================================================
    # INPUT ROW: MIC (optional) + hint
    # =========================================================
    mic_col, hint_col = st.columns([1, 4], vertical_alignment="center")
    with hint_col:
        st.caption("Tip: manda varios mensajes (ej. “3 cajas cerveza” + “2kg limones”). Luego parseas.")
    ...
    if audio_bytes and audio_hash and audio_hash != st.session_state.get("last_audio_hash"):
        try:
            ...
            _append_message("asr", transcript)
            st.success("✅ Audio transcrito y añadido al chat")
            st.rerun()

        except Exception as e:
            st.error(f"Error transcribiendo: {e}")

    # WhatsApp-like input at bottom
    typed = st.chat_input("Escribe un ítem del pedido… (ej: 3 cajas cerveza)")
    if typed:
        _append_message("user", typed)
        st.rerun()
    ...
    # =========================================================
    # =========================================================
    # Shared: PARSE  (FIX: parse per message, not one big blob)
    # =========================================================

    # Use chat messages as the source of truth (WhatsApp behavior)
    chat_msgs = [
        (m.get("text") or "").strip()
        for m in st.session_state.get("order_chat", [])
        if (m.get("text") or "").strip()
    ]

    # Fallback: if somehow chat is empty, use transcript_area
    ...
    if parse_clicked:
        # Build fragments from EACH message independently
        items: List[str] = []
        ...
        #--------------agg
        # --- AGGREGATE: sumar productos iguales (por product_id o matched_name) ---
        merged = {}

        for r in parsed_rows:
            matched_name = safe_str(r.get("matched_name")).strip()
            qty = float(r.get("quantity") or 0.0)
            unit = safe_str(r.get("unit")).strip().lower()
            conf = float(r.get("confidence") or 0.0)
            spoken = safe_str(r.get("spoken_name")).strip()

            # si no hay nada útil, saltar
            if not matched_name and qty == 0.0 and not unit:
                continue

            # clave: product_id si lo puedes resolver; si no, por nombre
            ...
            if key not in merged:
                merged[key] = {
                    "spoken_name": spoken,                    # puedes juntar varios si quieres
                ...
                # si la unidad estaba vacía, rellena
                if not (merged[key].get("unit") or "").strip():
                    merged[key]["unit"] = unit or "unidad"
                # spoken_name opcional: concat
...
def main():
    # --------------------------------
    # Run DB migrations only once
    # --------------------------------
    if not st.session_state.get("_orders_migrations_done", False):
        ensure_orders_migrations(engine)
        st.session_state["_orders_migrations_done"] = True
    init_auth_db()
