...
# Streamlit supports height as:
# - int (pixels)
# - "stretch"
# - "content"
# and it must NOT be None if passed.
Height = int | Literal["stretch", "content"] | None
...
def st_opaque_container(
    *,
    ...
    # ✅ IMPORTANT: Streamlit rejects height=None if passed
    if height is None:
        return opaque_container.container(border=border)
...
def st_fixed_container(
    *,
    ...
    with fixed_container:
        if transparent:
            # ✅ IMPORTANT: don't pass height=None into Streamlit
            if height is None:
